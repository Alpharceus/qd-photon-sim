...
def build_slides(verdict: dict, sweep_rows: list[dict], lv: DL.DotLevels,
    ...
    slides.append(Slide(
        ...
        [f"At the best {linewidth_sweep['split']:g} meV splitting, pulsed g2 rises with linewidth",
         f"The threshold is bracketed at gamma300={threshold} meV; 6.5 meV is the per-card anchor",
         "The primary GaAsP card never clears the flux floor; the conditional result is fallback-only",
    ...
    slides.append(Slide(
        ...
        [f"Flux margin is {flux_margin}; values above 1 clear the 1000 photons/s floor",
         "The primary GaAsP card never clears the flux floor; only the fallback card supplies the conditional result",
    ...
    slides.append(Slide(
        ...
        ["The stop rule is not met as stated: this is a conditional model corner, not a PASS",
         f"PASS needs the measured linewidth to land inside the {threshold} meV threshold bracket",
         "The primary card never clears the flux floor; the conditional result rests on fallback alone",
        ...
        "independent papers, and all hallucination tests passing) is not met. The 6.5 meV per-card "
        "evaluation passes only for the fallback card; the primary card remains below the flux "
        "floor, while evidence claims still lack independent support. This remains a "

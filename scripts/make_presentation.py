...
def build_slides(verdict: dict, sweep_rows: list[dict], lv: DL.DotLevels,
    ...
    slides.append(Slide(
        ...
        [f"At the best {linewidth_sweep['split']:g} meV splitting, pulsed g2 rises with linewidth",
         "The 0.5 line is crossed only at the low-linewidth end",
        ...
        "largest splitting; the Chatzarakis 2023 and Matsuda 2001 anchors are read from the shared "
        "evidence ledger. The linewidth is unmeasured for this InP platform.",
        image=figures["linewidth"], verbatim=[verdict["verdict_line"]]))

    slides.append(Slide(
        "Why brightness changes the answer",
        ["Flux floor: 1000 photons/s; 176 of 192 rows are excluded below it",
         "The declared HR back facet, NA 0.75 and 250 um ridge produce the eligible corner",
         f"Maximum collected flux is {brightness['reported collected_flux_pulsed_s']} photons/s"],
        f"The factor table is read from out/rt_edge/verdict.md: loading={brightness['loading = 1 - e^-mu (mu=1)']}, "
    ...
    slides.append(Slide(
        ...
        ["80 K model comparison: g2(0)=0.25 +/- 0.05 and about 4000 counts/s",
         "Reischle 2008: raw g2(0)=0.43; corrected g2(0)=0.03",
    ...
    slides.append(Slide(
        "The honest verdict",
        ["The stop rule is not met as stated: this is a conditional model corner, not a PASS",
         "PASS needs a measured 300 K InP-dot linewidth at or below about 7 meV",
        ...
        "independent papers, and all hallucination tests passing) is not met. Four eligible rows "
        "of the 192-row sweep clear the pulsed headline only at gamma300=6 meV and with declared "
        "collection levers, while evidence claims still lack independent support. This remains a "

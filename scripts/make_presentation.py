...
def evidence_numbers(path: Path) -> dict:
    ...
    return dict(passed=doc["checks_passed"], total=doc["checks_total"],
                hallucination_ok=doc["hallucination_tests_passed"])
...
def build_slides(verdict: dict, sweep_rows: list[dict], lv: DL.DotLevels,
                 ...
                 evidence: dict, anchors: dict, figures: dict) -> list[Slide]:
    ...
    slides.append(Slide(
        ...
        ["Sweep: 54 evaluated configurations, spanning the dot's linewidth, its X-XX splitting, and the detector response time",
         "Every corner is checked against three gates at once: pulsed g2, CW intrinsic g2, CW measured g2"],
        ...
        "dot.delta_xx in [4.0, 7.0] meV, dot.gamma300 in [6.0, 20.0] meV, and the detector response "
        "(IRF) in [50, 200] ps, evaluated for both the primary (GaAsP-well) and fallback (GaInP-"
        "well) design cards, each under pulsed and continuous-wave drive. The envelope figure shown "
        "here (out/rt_edge/envelope.png) is the sweep's own output, not redrawn.",
        image=ENVELOPE_PNG, verbatim=[verdict["verdict_line"]]))

    slides.append(Slide(
        "Results in detail: pulsed vs continuous drive",
        ["Pulsed drive can beat the literature ceiling at one extreme corner; continuous drive never does",
         "Carriers escape the dot in picoseconds at 300 K -- faster than any real detector can resolve"],
        f"{verdict['card_lines'][0]}\n{verdict['card_lines'][1]}\n\n{verdict['ceiling']}\n\n"
        "Why pulsed and continuous drive disagree so sharply: at 300 K, thermally activated carrier "
        "escape from the dot happens on a picosecond timescale (fsim_core.dot_levels retention "
        "model, InP/(Al,Ga)InP activation energies from Bommer et al. 2011 and Schulz et al., "
        "physica status solidi (c) 6, 906 (2009)). Under continuous electrical drive this makes the "
        "antibunching dip itself narrower than any real single-photon detector's 50-200 ps timing "
        "response, so the measured (IRF-convolved) CW g2 stays near 1 even though the intrinsic CW "
        "g2 can be favourable -- see the g2_cw0 vs g2_cw0_raw columns of out/rt_edge/sweep.csv. "
        "Pulsed operation, which asks only about the photon statistics within one short injection "
        "pulse, is required to see any single-photon character at all in this design.",
        image=ENVELOPE_PNG))

    slides.append(Slide(
        "Checking the numbers against the literature",
        ["Every literature number is tagged: verified, derived, class estimate, or assumption",
         f"An automatic check confirms every citation is real, in range and in context ({evidence['passed']}/{evidence['total']} checks passed)",
         "Two claims still have only one independent source and are honestly marked incomplete"],
    ...
    slides.append(Slide(
        "The honest verdict",
        ["This design does not pass the room-temperature stop rule",
         "Fail reasons: no corner clears all three gates together; two literature claims are still single-sourced",
         "No published dot has broken the g2 ≈ 0.5, 300 K ceiling -- electrical or optical"],
        f"{verdict['verdict_line']}\nFail reasons: " + ", ".join(verdict["fail_reasons"]) + ". "
        "The stop rule set at the start of this project (g2(0) < 0.5 at 300 K, working electrical "
        "injection, in-plane out-coupling, every claim cross-checked against at least two "
        "independent papers, and all hallucination tests passing) is not met: no single row of the "
        "54-point sweep clears the pulsed g2, CW intrinsic g2 and CW measured g2 gates "
        "simultaneously, and two evidence-ledger claims still rest on one source each. This is "
        "reported as a fail, not reframed as a conditional pass, because docs/rt_edge_contract.md "
        "makes evidence completeness and a same-row favourable corner both mandatory, and because "
        "no published III-V quantum dot -- at any drive scheme -- has demonstrated g2(0) < 0.5 at "
        "300 K under electrical driving.",
        verbatim=[verdict["verdict_line"]]))

    slides.append(Slide(
        "Limitations and what would have to change",
        ["A narrower 300 K linewidth or a larger X-XX splitting would help most (out/rt_edge/gui-smoke.png.txt)",
         "No InP/GaAsP single-dot measurement exists yet; today's linewidth and splitting are class proxies",
         "A second independent source would close the evidence gate"],

...
G = np.linspace(.2, 20, 250); fig, ax = plt.subplots(figsize=(1600/150, 900/150), dpi=150)
for d, c in zip([4, 6, 8], ['#1769aa', '#e08e0b', '#c43d3d']):
    ax.plot(G, [epsilon(d, g, g, w=g).eps for g in G], lw=3, color=c, label=f'$\\Delta_{{XX}}={d}$ meV')
ax.set(xlabel='FWHM $\\Gamma$ (meV)', ylabel='$\\varepsilon=t_{XX}/t_X$', title='Spectral overlap increases as thermal linewidth grows ($w=\\Gamma$)')

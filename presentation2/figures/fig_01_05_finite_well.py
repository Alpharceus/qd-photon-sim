...
# Parameters: V = 300 meV, width w = 10.0 nm, m_in = 0.08 m0, m_out = 0.12 m0
V_meV = 300.0
w_nm = 10.0
m_in = 0.08
m_out = 0.12
...
for idx, (E_bound, p) in enumerate(zip(well.energies_meV, well.parity)):
    color = '#1769aa' if p == 'even' else '#c43d3d'
    ax1.plot(E_bound, 0, 'o', color=color, markersize=10, zorder=5)
    y_offset = 0.4 if idx % 2 == 0 else -0.5
    ax1.annotate(f'{p.capitalize()}: {E_bound:.1f} meV',
                 xy=(E_bound, 0), xytext=(E_bound - 25, y_offset),
...
ax2.set_title(f'Bound States in 1D Finite Well (w = {w_nm:.0f} nm)', fontsize=16, fontweight='bold')

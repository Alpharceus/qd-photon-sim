...
# Divergence half-angles in radians and degrees
...
ax1.plot(theta_deg, Iy, color="#E53E3E", lw=2.8, linestyle="--", label=rf"Vertical $\theta_y$ (Wide, $\theta_{{div}} > 60^\circ$)")
...
ax2.plot(na_grid, eta_vals * 100, color="#2B6CB0", lw=2.8, label=r"$\eta_{NA}(\mathrm{NA})$ (Gaussian Far-Field [E])")
...
ax2.annotate(rf"$\mathrm{{NA}} = 0.80$: $\eta = {eta_80:.1f}\%$\n({gain_na:.2f}$\times$ Boost over NA 0.5)",
             xy=(0.80, eta_80), xytext=(0.48, 65),

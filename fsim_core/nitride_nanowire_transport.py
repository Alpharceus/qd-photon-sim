...
WIRE_RTH_PRESETS_K_W={"horizontal":1e9,"vertical":1e7,"deshpande_fig4_replay":3.1e9} # [DR, fit to both Fig.4 rises]: 3.1e9 K/W reproduces the +15 K@1nA and +49 K@2nA Deshpande 2013 Fig.4 heating estimates within the 15% acceptance band; it is fit to both rises, not independently measured.
...
def _kernel(d,I,T):
 ...
 'kernel_invalid_below_T' covers the planar kernel's two low-temperature
 underflow floors: the SRH j0 (NitrideDiode._j0 -- both the InGaN active
 and GaN spacer ni underflow to exactly 0, near ~25 K for the default
 composition) and the built-in potential's ni**2 (NitrideDiode.vbi, used
 inside depletion() -- only the larger-gap GaN ni need underflow there,
 so this floor sits HIGHER, near 57 K for the default composition [DR,
 measured by bisection against the public API in the verifier, not
 hardcoded here]; the Fig.4 replay anchor at T_hs=60 K is checked against
 that measured margin by the verifier, not assumed safe.
 'kernel_error' covers a non-finite result that reaches here without
 raising -- not attributable to either underflow floor.
 """
 k=d._kernel()
 try:
  if k._j0(T)==0.: return None,None,None,"kernel_invalid_below_T"
...
def _invalid(d,I,reason):
 ...
 return {"valid":False,"reasons":[reason],"provenance":dict(_PROVENANCE),
  "area_cm2":d.area_cm2,"J_A_cm2":I/d.area_cm2,"V_j":n,"V_terminal":n,"depletion_field_kVcm":n,"C_dep_F":n,
  "eta_inj":n,"r_e_leak_ratio":n,"r_h_leak_ratio":n,
  "f_capture":n,"f_qfl_dot":n,"f_qfl_background":n,

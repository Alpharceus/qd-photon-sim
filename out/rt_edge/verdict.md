...
Generated 2026-09-06T10:12:38.980415+00:00; contract: docs/rt_edge_contract.md.
...
|---|---:|---:|---:|---|---|
| edge-inp-gaasp-design | n/a (no headline-passing sample in this grid) | none(flux) | | | |
...
- **headline coverage over ELIGIBLE rows only** (same numerator, denominator restricted to eligible rows -- this is `coverage_over_eligible` in the VERDICT line, council review round 6 item 4, one name per quantity): 8/32 = 0.250
- pooled pulsed g2 median OVER ELIGIBLE ROWS ONLY (`g2_median_eligible` in the VERDICT line, the contract's median gate): 0.7019
- pooled pulsed g2 median over ALL VALID/DIAGNOSTIC rows (eligible rows plus rows excluded ONLY by the flux floor; `diag_g2_median_diagnostic` in the VERDICT line): 0.7352
